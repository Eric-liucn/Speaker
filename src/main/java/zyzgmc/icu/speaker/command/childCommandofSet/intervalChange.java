...
public class intervalChange implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        String newInterval = args.<String>getOne("间隔时间").get();
        if(!Config.rootNode.getNode("All",name).isVirtual()){
            ...
            src.sendMessage(TextSerializers.FORMATTING_CODE.deserialize(String.format("&a已将公告 &e%s &a的间隔时间改为 &e%s",name,newInterval)));
        }else {
        ...
        return CommandSpec.builder()
                ...
                .arguments(
                        GenericArguments.seq(
                                ...
                                GenericArguments.remainingJoinedStrings(Text.of("间隔时间"))
                        )
                )
