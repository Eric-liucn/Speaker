...
public class contentChange implements CommandExecutor {
    @Override
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {

        String name = args.<String>getOne("公告名称").get();
        String cont = args.<String>getOne("公告内容").get();
        if(!Config.rootNode.getNode("All",name).isVirtual()){
            Config.rootNode.getNode("All",name,"Content").setValue(cont);
            try{
                Config.save();
                Config.load();
            }catch (IOException e){
            ...
            src.sendMessage(TextSerializers.FORMATTING_CODE.deserialize(String.format("&a已将公告 &e%s &a的内容改为 %s",name,cont)));
        }else {
